...
const GroupInfo = () => {
  ...
  const getChatDetails = async () => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BSMS_BACKEND_URL}/api/Chat/getChatDetails/${chatId}`
      );
  ...
  const updateGroupChat = async (data) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BSMS_BACKEND_URL}/api/Chat/updateGroupChatInfo/${chatId}`,
        {
          method: "POST",
          headers: {
      ...
      if (res.ok) {
        router.push(`/dashboard/seller/conversation/RootChat/chats/${chatId}`);
      }
    } catch (error) {
      console.log(error);
    }
